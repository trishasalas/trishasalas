...
> **Update:** This exploration was formalized into a paper — [Accessibility Concept Emergence in the Pythia Suite](https://research.trishasalas.com/publications/accessibility-concept-emergence-pythia/) ([DOI](https://doi.org/10.5281/zenodo.20360787)).
...
|--------|-------|------|----|------| ------|
|A screen reader is| ❌ "view a screen" | ❌ "view a screen" | ⚠️ "reads text from a screen" | ✅ "reads aloud the text" | ✅ "reads text on a computer screen"|
|WCAG stands for|❌ hallucinated|❌ hallucinated|❌ hallucinated| ❌ hallucinated|✅ "Web Content Accessibility Guidelines"|
|A skip link is|❌ nonsense|❌ vague|❌ "data link"|✅ "skip a section"| ❌ "not displayed in browser"|
|The purpose of alt text is|❌ vague|❌ "customize"|❌ "add text"| ✅ "brief description"|✅ "description of the image for visually impaired"|
|ARIA stands for|❌ hallucinated |❌ loop|❌ hallucinated| ❌ hallucinated |❌ hallucinated|
|Captions are used for|❌ generic|⚠️ "description"|❌ generic| ⚠️ "photograph"|❌ generic|
...
| Prompt | 160M| 410M | 1B | 2.8B| 6.9B|
| --------------------------------------------------- | ------------------- | ------------------ | ---------------- | ----------------------------------------- | ----------------------------------------- |
| `<img src='photo.jpg'>` is missing | ❌| ❌ | ❌blank| ❌"."| ❌|
| A `<div>` with onclick is not accessible because | ❌| ❌ | ❌| ❌| ⚠️"not a form control"|
| `<input type='text'>` needs a| ❌"password" | ❌HTML soup | ❌"value" | ❌"value"| ❌"value"|
| A button that only says 'Click here' is bad because | ❌| ❌ | ❌| ✅"not clear what the button does" | ✅"not clear what the button does" |
...
**How to read this:** The visualization shows a grid where each row represents a token in the prompt. The darkness of each cell shows how much attention that token pays to previous tokens. When "reader" has a dark cell under "screen," it means the model is connecting these words together.
 ...
 alt='Attention pattern visualization showing token "reader" attending strongly to token "screen" in layer 31, head 23 of Pythia 2.8B model.'
 width={1280}
 height={720}
/>
